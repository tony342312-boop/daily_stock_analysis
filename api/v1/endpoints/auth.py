...
async def auth_register(request: Request, body: RegisterRequest):
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"error": "invalid_registration", "message": str(exc)},
        )
    session_user = {"id": user.id, "username": user.username, "role": user.role}
    session_val = create_session(session_user)
    resp = JSONResponse(content={"ok": True, "user": session_user})
    _set_session_cookie(resp, session_val, request)
    return resp
...
def _require_admin(request: Request):
    ...
    if current_user.get("role") != "admin":
        return JSONResponse(
            status_code=403,
            content={"error": "forbidden", "message": "仅管理员可执行此操作"},
        )
    return None
...
async def auth_update_user(user_id: int, request: Request, body: UserUpdateRequest):
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"error": "invalid_user_update", "message": str(exc)},
        )
