...
MIN_PASSWORD_LEN = 6
MAX_PASSWORD_LEN = 16
_current_user_ctx: contextvars.ContextVar[Optional[dict]] = contextvars.ContextVar(
    "dsa_current_user",
    default=None,
)
...
def _user_to_public_dict(user) -> dict:
    return {
        "id": user.id,
...
def ensure_admin_user(username: Optional[str] = None, password: Optional[str] = None) -> None:
    """Ensure configured admin account exists for multi-user login."""
...
def list_app_users() -> list:
    ...
    with db.get_session() as session:
        ...
        return [_user_to_public_dict(user) for user in users]
...
def create_session(user: Optional[dict] = None) -> str:
    """Create a signed session payload. Format: nonce.ts[.user_id.role.username].signature."""
    ...
    username = base64.urlsafe_b64encode(
        str((user or {}).get("username", "admin")).encode("utf-8")
    ).decode("ascii").rstrip("=")
